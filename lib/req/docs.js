...
function fetchDocsByDate(initialDate, finalDate, initialAuthData) {
    ...
    return requestPage(initialOptions, function (response) {
        ...
        return requestNextPages(currentPage, pagesNr, response, initialDate, finalDate);
    });
}

function requestNextPages(currPage, maxPage, previousPageResponse, initialDate, finalDate) {
    if (currPage >= maxPage) {
        Promise.resolve();
    }
    ...
    console.log(initialDate + ': ' +currPage + ' of ' + maxPage);
    return requestPage(nextPageOptions, function (nextPageResponse) {
        if (currPage < maxPage) {
            const docs = extractDocs(nextPageResponse);
            currPage = currPage + 1;
            return requestNextPages(currPage, maxPage, nextPageResponse, initialDate, finalDate);
        }
