...
function fetchDocsByDateAndAct(initialDate, finalDate, initialAuthData, act) {
    ...
    return requestPage(initialOptions, function (response) {
        ...
        return requestNextPages(currentPage, pagesNr, response, initialDate, finalDate, act);
    });
}

function requestNextPages(currPage, maxPage, previousPageResponse, initialDate, finalDate, act) {
    if (currPage >= maxPage) {
        Promise.resolve();
    }
    ...
    return requestPage(nextPageOptions, function (nextPageResponse) {
        if (currPage < maxPage) {
            ...
            currPage = currPage + 1;
            return requestNextPages(currPage, maxPage, nextPageResponse, initialDate, finalDate, act);
        }
