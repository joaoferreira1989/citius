...
function fetchDocsByDate(initialDate, finalDate, initialAuthData) {
    ...
    return requestPage(initialOptions, function (response) {
        ...
        return requestNextPage(currentPage, pagesNr, response, initialDate, finalDate)
            .then((nextPageResponse) => {
                currentPage++;

                return requestNextPage(currentPage, pagesNr, nextPageResponse, initialDate, finalDate);
            })
            .catch((e) => {
...
function requestNextPage(currPage, maxPage, previousPageResponse, initialDate, finalDate) {
    if (currPage >= maxPage) {
        Promise.resolve();
    }
...
console.log(currPage + ' of ' + maxPage);
    return requestPage(nextPageOptions, function (nextPageResponse) {
        if (currPage < maxPage) {
            const docs = extractDocs(nextPageResponse);
            currPage = currPage + 1;
            return requestNextPage(currPage, maxPage, nextPageResponse, initialDate, finalDate);
        }
