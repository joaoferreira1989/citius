...
function parseProcess(processHtml) {
    ...
    const act = processedInfo[3] ? processedInfo[3].trim() : undefined;
    const reference = processedInfo[5] ? processedInfo[5].trim() : undefined;
    const species = processedInfo[9] || undefined;
    const date = processedInfo[11] ? processedInfo[11] : undefined;

    const processData = processedInfo[7] ? processedInfo[7].replace('Processo:', '').trim() : '';
